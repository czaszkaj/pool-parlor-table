...
public class RepositionManager : UdonSharpBehaviour
{
    ...
    public void _Tick()
    {
        ...
        Vector3 k_pR = (Vector3)table.managers.currentPhysicsManager.GetProgramVariable("k_pR");
        Vector3 k_pO = (Vector3)table.managers.currentPhysicsManager.GetProgramVariable("k_pO");
        ...
        for (int i = 0; i < repositioning.Length; i++)
        {
            ...
            else if (table.playerManager.IsLocalPlayerReferee())
            {
                maxX = k_pR.x;
            }
            else
